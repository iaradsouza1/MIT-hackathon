...
def main():
    ...
    if page == 'Nation-wide statistics':
        ...
        st.plotly_chart(plots.bar_plots(df_results, last_day))

    elif page == 'By state statistics':
        state = st.sidebar.selectbox('Select a state:', states_names)

        st.header('Check new cases by day')
        st.plotly_chart(plots.plot_new_cases(state, last_day))

        st.header('Check Rt by day')    
        st.plotly_chart(plots.plot_rt_state(state, df_results, last_day))
